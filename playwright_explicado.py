...
async def main():
    async with async_playwright() as p: # para inicializar o playwright precisa desse "async with" por questoes de seguranca,
        ...
        browser = await p.chromium.launch()# ele vem com headless=True, que faz com que o navegador abra durante a execucao, mas caso queiram so colocar no launch(headless=False)
        page = await browser.new_page() # criando a page, que e aonde voce navega,Clica,Le conteudo, executa JavaScript
        await page.goto("https://bileto.sympla.com.br/event/115660/d/362549") # Acessando a URL
        # quando voce nao define wait_until e o timeout, o proprio playwright decide por voce, o default é await page.goto(url, wait_until="load", timeout=30000) e voce pode mudar caso necessario
        ...
        print(precos)
        await browser.close() # fechando a janela

asyncio.run(main()) # para executar precisa usar o asyncio, para fazer o script ser executado de maneira assincrona porque TUDO que conversa com o navegador é ASSINCRONO
